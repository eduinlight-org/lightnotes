...
pub async fn run(store: StoreHandle, offline: Signal<bool>, active_user: Signal<Option<String>>) {
  ...
  loop {
    ...
    let Some(user_id) = active_user.peek().clone() else {
      tokio::time::sleep(Duration::from_millis(BASE_BACKOFF_MS)).await;
      continue;
    };

    let Some((row_id, change)) = store.peek_front_outbound(&user_id).await else {
      tokio::time::sleep(Duration::from_millis(BASE_BACKOFF_MS)).await;
      continue;
    };
